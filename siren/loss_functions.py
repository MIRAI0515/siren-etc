...
'''画像の予測と実際の画像の間の平均二乗誤差(MSE)を計算します。'''
# mask: マスク（オプション）。
# model_output: モデルの出力。
# gt: 実際の画像（グラウンドトゥルース）。
# 戻り値: 損失辞書。
...
'''画像の予測と実際の画像の間の平均絶対誤差(L1)を計算します。'''
# 引数：image_mseと同様。
# 戻り値: 損失辞書。
...
'''MSEに加えて、全変動(Total Variation)の正則化項を持つ損失を計算します。'''
# 引数：k1: 正則化項の重み。他はimage_mseと同様。
# 戻り値: 損失辞書。
...
def sdf(model_output, gt):
    ...
    """
    return {'sdf': torch.abs(sdf_constraint).mean() * 3e3,  # 1e4      # 3e3
            'inter': inter_constraint.mean() * 1e2,  # 1e2 
            'grad_constraint': grad_constraint.mean() * 5e1}  # 1e1      # 5e1
    """
