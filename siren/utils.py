...
from skimage.metrics import structural_similarity as compare_ssim #yasudaコメントアウトした
...
'''この関数 write_image_summary は、画像生成モデルの評価結果をTensorBoardに書き込むためのものです。
具体的には、モデルの出力画像とそれに対応するグラウンドトゥルース（真値）画像、およびそれらの勾配とラプラシアンを可視化して記録します。'''
# image_resolution: 画像の解像度。
# model: 使用しているモデル。
# model_input: モデルへの入力データ。
# gt: グラウンドトゥルース（真値）データ。
# model_output: モデルの出力データ。
# writer: TensorBoardのライターオブジェクト。
# total_steps: トレーニングのステップ数。
# prefix: 画像名のプレフィックス（デフォルトは 'train_'）。
def write_image_summary(image_resolution, model, model_input, gt,
                        model_output, writer, total_steps, prefix='train_'):
    
    # gt_img と pred_img をリニア形式から画像形式に変換。
    gt_img = dataio.lin2img(gt['img'], image_resolution)
    pred_img = dataio.lin2img(model_output['model_out'], image_resolution)
    ...
    # 予測画像とグラウンドトゥルース画像を結合してTensorBoardに書き込む。
    output_vs_gt = torch.cat((gt_img, pred_img), dim=-1)
    writer.add_image(prefix + 'gt_vs_pred', make_grid(output_vs_gt, scale_each=False, normalize=True),
                     global_step=total_steps)

    # 予測画像、勾配、ラプラシアンを再スケーリングし、NumPy配列に変換。
    ...
    # グラウンドトゥルース画像、勾配、ラプラシアンを再スケーリングし、NumPy配列に変換。
    ...
    # 予測画像、勾配、ラプラシアン、およびそれぞれのグラウンドトゥルースをTensorBoardに書き込む。
    ...
    # 予測画像とグラウンドトゥルース画像のピーク信号対雑音比(PSNR)を計算し、TensorBoardに書き込む。
...
'''
予測画像とグラウンドトゥルース（真値）画像の間のPSNR（ピーク信号対雑音比）とSSIM（構造類似度指数）を計算し、
その結果をTensorBoardに書き込むためのものです。'''
'''この関数は、予測画像とグラウンドトゥルース画像のPSNRとSSIMを計算してTensorBoardに記録することを目的としていますが、
実際のPSNRとSSIMの計算部分がコメントアウトされており、代わりに0が代入されています。このため、現在の状態ではPSNRとSSIMの値は常に0になってしまいます。コメントアウトされている部分を有効にし、正しいPSNRとSSIMの計算を行う必要があります。'''
# pred_img: モデルが生成した予測画像。
# gt_img: グラウンドトゥルース画像。
# writer: TensorBoardのライターオブジェクト。
# iter: 現在のトレーニングステップ数。
# prefix: 指標名のプレフィックス。
def write_psnr(pred_img, gt_img, writer, iter, prefix):
    ...
    for i in range(batch_size):
        ...
        # ここで本来はPSNRとSSIMを計算するが、コメントアウトされている。
        #ssim = skimage.metrics.compare_ssim(p, trgt, multichannel=True, data_range=1) #(0613)skimage.metrics.によりエラー(yasuda)
        #psnr = skimage.metrics.compare_psnr(p, trgt, data_range=1) #(0613)skimage.metrics.によりエラー(yasuda)
        
        ssim = compare_ssim(p, trgt, channel_axis=-1, data_range=1) #(0617)元のコードだと動かないため、実際の計算が行わないようにコメントアウト。(0613)ミーティングで直していただいた部分
        psnr = compare_psnr(p, trgt, data_range=1) #(0617)元のコードだと動かないため、実際の計算が行わないようにコメントアウト。(0613)ミーティングで直していただいた部分
        
        # ここでは、PSNRとSSIMの計算が0で初期化されており、実際の計算が行われていない。
        #ssim = 0 #(0617)元のコードだと動かないため、実際の計算が行わないようにした
        #psnr = 0 #(0617)元のコードだと動かないため、実際の計算が行わないようにした
